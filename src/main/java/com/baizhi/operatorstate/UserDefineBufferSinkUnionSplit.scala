...
class UserDefineBufferSinkUnionSplit(threshold: Int = 0) extends SinkFunction[(String, Int)] with CheckpointedFunction{
  ...
  override def initializeState(context: FunctionInitializationContext): Unit = {
    ...
    checkpointedState = context.getOperatorStateStore.getUnionListState(lsd) //默认广播方式恢复
